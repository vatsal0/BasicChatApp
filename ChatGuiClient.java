...
public class ChatGuiClient extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        stage.setOnCloseRequest(e -> {
            try { out.writeObject(new ChatMessage(ChatMessage.HEADER_QUIT)); } catch (Exception ex) {ex.printStackTrace();}
    ...
    private void sendMessage() {
        ...
        if (message.length() == 0)
            return;
        textInput.clear();
        try { out.writeObject(new ChatMessage(ChatMessage.HEADER_CHAT, message)); } catch (Exception ex) {ex.printStackTrace();}
    }
